...
Apply filters and normalization to clean up data before analysis.

#### Recommended Workflow:
1. Normalize data (e.g., Log2) to adjust for technical variation
2. Apply peptide count filter (≥ 2) to remove unreliable proteins
3. Require > 50% valid values within each group
4. Use "Min" imputation for remaining missing values

#### About Peptide Count Filter:
This filter works with Spectronaut format or columns renamed to match your quantitative columns with "PG.NrOfStrippedSequencesIdentified" appended.

#### About CV Filter:
The coefficient of variation (CV) filter helps remove proteins with high variability within groups.
""")
...
if "data" not in st.session_state or st.session_state.data is None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs([
        "1. Normalization", "2. Peptide Count Filter", "3. Valid Values Filter",
        "4. CV Threshold"
    ])
    ...
    # Store original data for normalization reversal
    if "original_data_before_normalization" not in st.session_state:
        st.session_state.original_data_before_normalization = df.copy()
    ...
    with tab1:
        ...
        if st.button("Apply Normalization"):
            # Start from the original uploaded data for normalization
            ...
            if norm_method != "None":
                ...
                normalized_df = DataProcessor.normalize_data(base_data, norm_method, numeric_cols)
                
                st.session_state.filtered_data = normalized_df
                ...
                st.session_state.filtered_data = base_data.copy()
                st.session_state.normalization_method = None
                st.success("Normalization removed, using original data")
            
            # Update the reference point for normalization
            st.session_state.original_data_before_normalization = st.session_state.filtered_data.copy()
    ...
    with tab3:
        ...
        if not group_selections:
            ...
            imputation_method = st.selectbox(
                ...
                ["None", "Mean", "Median", "Min", "KNN", "Group-wise Mean", "Truncated Normal"],
                help="""
                ...
                Truncated Normal: Replace with values from a truncated normal distribution centered at 1st percentile with 20% CV
                """)
            ...
            if impute_button:
                if imputation_method != "None":
                    # Check if data has been log2 normalized
                    ...
                    imputed_df = DataProcessor.impute_missing_values(
                        ...
                        group_selections if imputation_method == "Group-wise Mean" else None,
                        is_log2_normalized=is_log2_normalized)
